...
def test_vector_core_dependency_is_pinned_to_v1_4_3() -> None:
    ...
    assert vector_specs == [
        "vector-core @ git+https://github.com/michaelkrauty/vector-core.git@v1.5.0"
    ]

    uv_sources = pyproject.get("tool", {}).get("uv", {}).get("sources", {})
    assert uv_sources.get("vector-core", {}).get("tag") == "v1.5.0"


def test_readme_install_example_uses_current_vector_core_pin() -> None:
    readme = (ROOT / "README.md").read_text(encoding="utf-8")

    assert "vector-core.git@v1.5.0" in readme
